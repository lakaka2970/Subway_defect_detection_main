...
from subway_yolo.nn.autobackend import check_class_names
from subway_yolo.nn.modules import (
    AIFI,
    C1,
    C2,
    C2PSA,
...
from subway_yolo.utils import DEFAULT_CFG_DICT, LOGGER, WINDOWS, YAML, colorstr, emojis
from subway_yolo.utils.checks import check_requirements, check_suffix, check_yaml
from subway_yolo.utils.loss import (
    E2ELoss,
    PoseLoss26,
...
from subway_yolo.utils.ops import make_divisible
from subway_yolo.utils.patches import torch_load
from subway_yolo.utils.plotting import feature_visualization
...
class DetectionModel(BaseModel):
    ...
    def _descale_pred(p, flips, scale, img_size, dim=1):
        ...
        Args:
            ...
            flips (Optional[int]): Flip type (None=none, 2=ud, 3=lr).
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        """
        from subway_yolo.nn.text_model import build_text_model
...
class YOLOEModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from subway_yolo.utils.loss import TVPDetectLoss
...
class YOLOESegModel(YOLOEModel, SegmentationModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from subway_yolo.utils.loss import TVPSegmentLoss
...
def torch_safe_load(weight, safe_only=False):
    ...
    Args:
        weight (Union[str, Path]): The file path of the PyTorch model.
    ...
    Examples:
        >>> from subway_yolo.nn.tasks import torch_safe_load
        >>> ckpt, file = torch_safe_load("path/to/best.pt", safe_only=True)
    """
    from subway_yolo.utils.downloads import attempt_download_asset
...
def load_checkpoint(weight, device=None, inplace=True, fuse=False):
    ...
    Args:
        weight (Union[str, Path]): Model weight path.
...
def guess_model_scale(model_path):
    ...
    Args:
        model_path (Union[str, Path]): The path to the YOLO model's YAML file.
...
def guess_model_task(model):
    ...
    Args:
        model (torch.nn.Module | dict | Union[str, Path]): PyTorch model, model configuration dict, or model file path.
