...
from subway_yolo.utils.checks import check_suffix
from subway_yolo.utils.downloads import is_url
...
def check_class_names(names: list | dict) -> Dict[int, str]:
    ...
    if isinstance(names, dict):
        ...
        if isinstance(names[0], str) and names[0].startswith("n0"):  # imagenet class codes, i.e. 'n01440764'
            from subway_yolo.utils import ROOT, YAML
...
def default_class_names(data: Union[str, Path] | None = None) -> Dict[int, str]:
    """Load class names from a YAML file or return numerical class names.

    Args:
        data (Union[str, Path], optional): Path to YAML file containing class names.
    ...
    if data:
        try:
            from subway_yolo.utils import YAML
            from subway_yolo.utils.checks import check_yaml
...
class AutoBackend(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            data (Union[str, Path], optional): Path to the additional data.yaml file containing class names.
    ...
    def warmup(self, imgsz: tuple[int, int, int, int] = (1, 3, 640, 640)) -> None:
        ...
        """
        from subway_yolo.utils.nms import non_max_suppression
