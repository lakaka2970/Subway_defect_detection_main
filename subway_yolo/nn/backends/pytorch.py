...
from subway_yolo.utils import IS_JETSON, LOGGER, is_jetson
...
class PyTorchBackend(BaseBackend):
    ...
    def __init__(
        self,
        weight: Union[str, Path] | nn.Module,
        device: torch.device,
        fp16: bool = False,
        fuse: bool = True,
        verbose: bool = True,
        ...
        Args:
            weight (Union[str, Path] | nn.Module): Path to the .pt model file or a pre-loaded nn.Module instance.
...
class TorchScriptBackend(BaseBackend):
    ...
    def __init__(self, weight: Union[str, Path], device: torch.device, fp16: bool = False):
        """Initialize the TorchScript backend.

        Args:
            weight (Union[str, Path]): Path to the .torchscript model file.
            device (torch.device): Device to run inference on (e.g., 'cpu', 'cuda:0').
            fp16 (bool): Whether to use FP16 half-precision inference.
        """
        super().__init__(weight, device, fp16)
