...
class ONNXBackend(BaseBackend):
    ...
    def __init__(self, weight: Union[str, Path], device: torch.device, fp16: bool = False, format: str = "onnx"):
        """Initialize the ONNX backend.

        Args:
            weight (Union[str, Path]): Path to the .onnx model file.
...
class ONNXIMXBackend(ONNXBackend):
    ...
    def load_model(self, weight: Union[str, Path]) -> None:
        ...
        Args:
            weight (Union[str, Path]): Path to the IMX model directory containing the .onnx file.
        """
