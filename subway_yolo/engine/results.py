...
from subway_yolo.utils import LOGGER, DataExportMixin, SimpleClass, ops
from subway_yolo.utils.plotting import Annotator, colors, save_one_box
...
class BaseTensor(SimpleClass):
    ...
    def cuda(self):
        ...
        Examples:
            >>> import torch
            >>> from subway_yolo.engine.results import BaseTensor
            >>> data = torch.tensor([[1, 2, 3], [4, 5, 6]])
            >>> base_tensor = BaseTensor(data, orig_shape=(720, 1280))
            >>> gpu_tensor = base_tensor.cuda()
...
class Results(SimpleClass, DataExportMixin):
    ...
    def plot(
        ...
        Args:
            ...
            line_width (Optional[float]): Line width of bounding boxes. If None, scaled to image size.
            font_size (Optional[float]): Font size for text. If None, scaled to image size.
            ...
            filename (Optional[str]): Filename to save image if save is True.
    ...
    def save(self, filename: Optional[str] = None, *args, **kwargs) -> str:
        ...
        Args:
            filename (Optional[str]): The filename to save the annotated image. If None, a default filename is generated
    ...
    def save_txt(self, txt_file: Union[str, Path], save_conf: bool = False) -> str:
        ...
        Args:
            txt_file (Union[str, Path]): Path to the output text file.
    ...
    def save_crop(self, save_dir: Union[str, Path], file_name: Union[str, Path] = Path("im.jpg")):
        ...
        Args:
            save_dir (Union[str, Path]): Directory path where cropped images will be saved.
            file_name (Union[str, Path]): Base filename for the saved cropped images.
...
class Keypoints(BaseTensor):
    ...
    Examples:
        >>> import torch
        >>> from subway_yolo.engine.results import Keypoints
