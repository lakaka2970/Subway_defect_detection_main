...
from subway_yolo.cfg import get_cfg, get_save_dir
from subway_yolo.data import load_inference_source
from subway_yolo.data.augment import LetterBox
from subway_yolo.nn.autobackend import AutoBackend
from subway_yolo.utils import DEFAULT_CFG, LOGGER, MACOS, WINDOWS, callbacks, colorstr, ops
from subway_yolo.utils.checks import check_imgsz, check_imshow
from subway_yolo.utils.files import increment_path
from subway_yolo.utils.torch_utils import attempt_compile, select_device, smart_inference_mode
...
class BasePredictor:
    ...
    def __init__(
        ...
        Args:
            cfg (Union[str, Path] | dict | SimpleNamespace): Path to a configuration file or a configuration dictionary.
    ...
    def __call__(self, source=None, model=None, stream: bool = False, *args, **kwargs):
        ...
        Args:
            source (Union[str, Path] | List[str] | List[Path] | List[np.ndarray] | np.ndarray | torch.Tensor, optional):
                Source for inference.
            model (Union[str, Path] | torch.nn.Module, optional): Model for inference.
    ...
    def setup_source(self, source, stride: Optional[int] = None):
        """Set up source and inference mode.

        Args:
            source (Union[str, Path] | List[str] | List[Path] | List[np.ndarray] | np.ndarray | torch.Tensor): Source for
                inference.
    ...
    def setup_model(self, model, verbose: bool = True):
        ...
        Args:
            model (Union[str, Path] | torch.nn.Module): Model to load or use.
