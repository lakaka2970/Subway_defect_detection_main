...
def spaces_in_path(path: Union[str, Path]):
    ...
    Args:
        path (Union[str, Path]): The original path that may contain spaces.
...
def increment_path(path: Union[str, Path], exist_ok: bool = False, sep: str = "", mkdir: bool = False) -> Path:
    ...
    Args:
        path (Union[str, Path]): Path to increment.
...
def update_models(model_names: tuple = ("yolo26n.pt",), source_dir: Path = Path("."), update_names: bool = False):
    ...
    Examples:
        ...
        >>> from subway_yolo.utils.files import update_models
        >>> model_names = ("yolo26n.pt", "yolo11s.pt")
        >>> update_models(model_names, source_dir=Path("/models"), update_names=True)
    """
    from subway_yolo import YOLO
    from subway_yolo.nn.autobackend import default_class_names
