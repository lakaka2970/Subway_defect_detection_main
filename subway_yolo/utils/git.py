...
class GitRepo:
    ...
    Attributes:
        ...
        head (Optional[str]): Raw contents of HEAD; a SHA for detached HEAD or "ref: <refname>" for branch heads.
        is_repo (bool): Whether the provided path resides inside a Git repository.
        branch (Optional[str]): Current branch name when HEAD points to a branch; None for detached HEAD or non-repo.
        commit (Optional[str]): Current commit SHA for HEAD; None if not determinable.
        origin (Optional[str]): URL of the "origin" remote as read from gitdir/config; None if unset or unavailable.
    ...
    @staticmethod
    def _read(p: Path | None) -> Optional[str]:
    ...
    def _ref_commit(self, ref: str) -> Optional[str]:
