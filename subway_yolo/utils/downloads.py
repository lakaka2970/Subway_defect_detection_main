...
from subway_yolo.utils import ASSETS_URL, LOGGER, TQDM, checks, clean_url, emojis, is_online, url2file
...
GITHUB_ASSETS_NAMES = frozenset(
    ...
    # yolo26* models are not published on ultralytics/assets — use yolo11* instead
    ...
    # yolo26* models are not published on ultralytics/assets — use yoloe-11* instead
...
def is_url(url: Union[str, Path], check: bool = False) -> bool:
    """Validate if the given string is a URL and optionally check if the URL exists online.

    Args:
        url (Union[str, Path]): The string to be validated as a URL.
...
def delete_dsstore(path: Union[str, Path], files_to_delete: tuple[str, ...] = (".DS_Store", "__MACOSX")) -> None:
    """Delete all specified system files in a directory.

    Args:
        path (Union[str, Path]): The directory path where the files should be deleted.
        files_to_delete (tuple[str, ...]): The files to be deleted.

    Examples:
        >>> from subway_yolo.utils.downloads import delete_dsstore
...
def zip_directory(
    ...
    Args:
        directory (Union[str, Path]): The path to the directory to be zipped.
...
def unzip_file(
    ...
    Args:
        file (Union[str, Path]): The path to the zipfile to be extracted.
        path (Union[str, Path], optional): The path to extract the zipfile to.
        exclude (tuple[str, ...], optional): A tuple of filename strings to be excluded.
    ...
    Examples:
        >>> from subway_yolo.utils.downloads import unzip_file
...
def check_disk_space(
    ...
    Args:
        ...
        path (Union[str, Path], optional): The path or drive to check the available free space on.
...
def get_google_drive_file_info(link: str) -> tuple[str, Optional[str]]:
    ...
    Returns:
        ...
        filename (Optional[str]): Original filename of the Google Drive file. If filename extraction fails, returns None.

    Examples:
        >>> from subway_yolo.utils.downloads import get_google_drive_file_info
...
def safe_download(
    ...
    Args:
        url (Union[str, Path]): The URL of the file to be downloaded.
        file (Union[str, Path], optional): The filename of the downloaded file. If not provided, the file will be saved with
            the same name as the URL.
        dir (Union[str, Path], optional): The directory to save the downloaded file. If not provided, the file will be saved
...
def attempt_download_asset(
    ...
    **kwargs,
    ...
    Args:
        file (Union[str, Path]): The filename or file path to be downloaded.
        repo (str, optional): The GitHub repository in the format 'owner/repo'.
    ...
    """
    from subway_yolo.utils import SETTINGS  # scoped for circular import
    ...
    if file.exists():
        ...
        # yolo11* equivalent and resolve from weights/ first.
        ...
        if name.startswith(_BLOCKED_PREFIXES) and name.endswith(".pt"):
            ...
            # Prefer weights/ copy first, then fall back to download
