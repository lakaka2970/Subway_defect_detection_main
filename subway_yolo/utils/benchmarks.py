...
Usage:
    from subway_yolo.utils.benchmarks import ProfileModels, benchmark
...
from subway_yolo import YOLO, YOLOWorld
from subway_yolo.cfg import TASK2DATA, TASK2METRIC
from subway_yolo.engine.exporter import export_formats
from subway_yolo.nn.modules import Segment26
...
from subway_yolo.utils.checks import IS_PYTHON_3_13, check_imgsz, check_requirements, check_yolo, is_rockchip
from subway_yolo.utils.downloads import safe_download
from subway_yolo.utils.files import file_size
from subway_yolo.utils.torch_utils import get_cpu_info, select_device
...
def benchmark(
    ...
    **kwargs,
    ...
    Args:
        model (Union[str, Path]): Path to the model file or directory.
        data (Optional[str]): Dataset to evaluate on, inherited from TASK2DATA if not passed.
...
class ProfileModels:
    ...
    def __init__(
        ...
        half: bool = True,
        trt: bool = True,
        device: torch.device | Optional[str] = None,
    ):
        ...
        Args:
            ...
            half (bool): Flag to indicate whether to use FP16 half-precision for TensorRT profiling.
            trt (bool): Flag to indicate whether to profile using TensorRT.
            device (torch.device | Optional[str]): Device used for profiling. If None, it is determined automatically.
