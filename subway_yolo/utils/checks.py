...
def parse_requirements(file_path=ROOT.parent / "requirements.txt", package=""):
    ...
    Examples:
        >>> from subway_yolo.utils.checks import parse_requirements
...
def check_latest_pypi_version(package_name="ultralytics"):
    ...
    Returns:
        (Optional[str]): The latest version of the package, or None if unavailable.
    """
...
def check_font(font="Arial.ttf"):
    ...
    """
    from matplotlib import font_manager  # scope for faster 'import subway_yolo'
...
def check_model_file_from_stem(model: str = "yolo11n") -> Union[str, Path]:
    ...
    Returns:
        (Union[str, Path]): Model filename with appropriate suffix.
    """
    path = Path(model)
...
def check_file(file, suffix="", download=True, download_dir=".", hard=True):
    ...
    if (
        ...
        from subway_yolo.utils.callbacks.platform import resolve_platform_uri
...
def check_yaml(file, suffix=(".yaml", ".yml"), hard=True):
    ...
    Args:
        file (Union[str, Path]): File name or path.
...
def check_yolo(verbose=True, device=""):
    ...
    from subway_yolo.utils.torch_utils import select_device
...
def collect_system_info():
    ...
    from subway_yolo.utils import ENVIRONMENT  # scope to avoid circular import
    from subway_yolo.utils.torch_utils import get_cpu_info, get_gpu_info
...
def check_amp(model):
    ...
    Examples:
        ...
        >>> from subway_yolo.utils.checks import check_amp
    ...
    # Fallback: generate a dummy image if bus.jpg is missing (vendored setup)
    if not im.exists():
        import numpy as np
    ...
    try:
        ...
        LOGGER.warning(f"{prefix}checks skipped. Offline and unable to download model for AMP checks. {warning_msg}")
    except (AttributeError, ModuleNotFoundError):
        LOGGER.warning(
            f"{prefix}checks skipped. "
            f"Unable to load model for AMP checks due to possible package modifications. {warning_msg}"
        )
    except RuntimeError:
        LOGGER.warning(
            f"{prefix}checks skipped. "
            f"Failed to load model for AMP checks (checkpoint may be corrupted). {warning_msg}"
        )
...
def is_intel():
    ...
    """
    from subway_yolo.utils.torch_utils import get_cpu_info
