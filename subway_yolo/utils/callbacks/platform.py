...
from subway_yolo.utils import ENVIRONMENT, GIT, LOGGER, PYTHON_VERSION, RANK, SETTINGS, TESTS_RUNNING, Retry, colorstr
...
try:
    ...
    from subway_yolo.utils.logger import ConsoleLogger, SystemLogger
    from subway_yolo.utils.torch_utils import model_info_for_loggers
...
def resolve_platform_uri(uri, hard=True):
    ...
    Returns:
        (Optional[str]): Signed URL on success, None if not found and hard=False.
...
def _upload_model(model_path, project, name, progress=False, retry=1, model_id=None):
    ...
    from subway_yolo.utils.uploads import safe_upload
