...
from subway_yolo.utils import LOGGER, MACOS, RANK
...
class ConsoleLogger:
    ...
    Attributes:
        destination (Union[str, Path] | None): Target destination for streaming (URL, Path, or None for callback-only).
    ...
    def __init__(self, destination=None, batch_size=1, flush_interval=5.0, on_flush=None):
        ...
        Args:
            destination (Union[str, Path] | None): API endpoint URL (http/https), local file path, or None.
