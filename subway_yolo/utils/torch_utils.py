...
from subway_yolo.utils.checks import check_version
from subway_yolo.utils.cpu import CPUInfo
...
def get_cpu_info():
    ...
    from subway_yolo.utils import PERSISTENT_CACHE  # avoid circular import error
...
def strip_optimizer(f: Union[str, Path] = "best.pt", s: str = "", updates: Dict[str, Any] | None = None) -> Dict[str, Any]:
    """Strip optimizer from 'f' to finalize training, optionally save as 's'.

    Args:
        f (Union[str, Path]): File path to model to strip the optimizer from.
    ...
    Examples:
        >>> from pathlib import Path
        >>> from subway_yolo.utils.torch_utils import strip_optimizer
...
def profile_ops(input, ops, n=10, device=None, max_num_obj=0):
    ...
    Examples:
        >>> from subway_yolo.utils.torch_utils import profile_ops
