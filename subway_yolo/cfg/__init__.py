...
def cfg2dict(cfg: Union[str, Path] | dict | SimpleNamespace) -> dict:
    ...
    Args:
        cfg (Union[str, Path] | dict | SimpleNamespace): Configuration object to be converted. Can be a file path, a string, a
...
def get_cfg(
    cfg: Union[str, Path] | dict | SimpleNamespace = DEFAULT_CFG_DICT, overrides: dict | None = None
) -> SimpleNamespace:
    """Load and merge configuration data from a file or dictionary, with optional overrides.

    Args:
        cfg (Union[str, Path] | dict | SimpleNamespace): Configuration data source. Can be a file path, dictionary, or
...
def get_save_dir(args: SimpleNamespace, name: Optional[str] = None) -> Path:
    ...
    Args:
        args (SimpleNamespace): Namespace object containing configurations such as 'project', 'name', 'task', 'mode',
            and 'save_dir'.
        name (Optional[str]): Optional name for the output directory. If not provided, it defaults to 'args.name' or the
            'args.mode'.
...
def handle_yolo_hub(args: List[str]) -> None:
    ...
    Notes:
        - The function imports the 'hub' module from subway_yolo to perform login and logout operations.
        - For the 'login' command, if no API key is provided, an empty string is passed to the login function.
        - The 'logout' command does not require any additional arguments.
    """
    from subway_yolo import hub
...
def handle_yolo_solutions(args: List[str]) -> None:
    ...
    """
    from subway_yolo.solutions.config import SolutionConfig
...
def entrypoint(debug: str = "") -> None:
    ...
    if "rtdetr" in stem:  # guess architecture
        from subway_yolo import RTDETR

        model = RTDETR(model)  # no task argument
    elif "fastsam" in stem:
        from subway_yolo import FastSAM
