...
def _crop_image_paths(crop_dir: Path) -> tuple[str, str, bool]:
    """Return explicit train/val image paths for a YOLO crop dataset."""
    train_images = crop_dir / "train" / "images"
    val_images = crop_dir / "val" / "images"
    if train_images.is_dir():
        return (
            str(train_images.resolve()),
            str(val_images.resolve()) if val_images.is_dir() else str(train_images.resolve()),
            True,
        )
    legacy_train = crop_dir / "images" / "train"
    legacy_val = crop_dir / "images" / "val"
    if legacy_train.is_dir():
        return (
            str(legacy_train.resolve()),
            str(legacy_val.resolve()) if legacy_val.is_dir() else str(legacy_train.resolve()),
            True,
        )
    return (
        str((crop_dir / "train" / "images").resolve()),
        str((crop_dir / "val" / "images").resolve()),
        False,
    )
...
def generate_stage2_domain_adapt(root: Path, dry_run: bool = False) -> Optional[Path]:
    ...
    subway_dir = Path("data/subway_crops_1024")
    train_path, val_path, found = _crop_image_paths(subway_dir)
    if not found:
        print(f"  [WARN] Stage 2: 1024 crop dataset not found at {subway_dir}")
...
def generate_stage3_main_training(root: Path, dry_run: bool = False) -> Optional[Path]:
    ...
    subway_dir = Path("data/subway_crops")
    train_path, val_path, _ = _crop_image_paths(subway_dir)
