...
class DatasetBuilder:
    ...
    def _existing_converted_outputs(self, outputs: Dict[str, Path]) -> Dict[str, Path]:
        """Include already-converted YOLO datasets under public/.

        This supports the documented manual workflow:
        place prepared datasets in data/multi_datasets/public/<key>/ and run
        with --no-download. These directories must not be passed through the
        converters again because convert_all() cleans stale conversion output.
        """
        for key, spec in self._enabled_specs().items():
            if key in outputs or spec.target_class not in (GENERIC_DEFECT_CLASS, TINY_OBJECT_CLASS):
                continue
            out = self.public_dir / key
            required = [
                out / "images" / "train",
            ...
            if not all(p.is_dir() for p in required):
                continue
            train_images = list((out / "images" / "train").glob("*.jpg")) + list((out / "images" / "train").glob("*.png"))
            val_images = list((out / "images" / "val").glob("*.jpg")) + list((out / "images" / "val").glob("*.png"))
            ...
            if train_images and val_images and train_labels and val_labels:
                outputs[key] = out
                print(ok(f"[{key}] Using existing converted dataset at {out}"))
        return outputs
    ...
    def run(self) -> Dict[str, Path]:
        ...
        outputs = self._existing_converted_outputs(outputs)
