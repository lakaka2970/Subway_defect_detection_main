...
def generate_crops(
    ...
    # Include both labelled sources and true negative sources.  The latter are
    # first-class training data in the 2026-07-14 dataset and must not be
    # silently discarded merely because no .txt file exists.
    img_stems = sorted({
        p.stem for p in img_dir.iterdir()
        if p.is_file() and p.suffix.lower() in {".jpg", ".jpeg", ".png", ".bmp"}
    })
    if source_list:
        requested = {line.strip() for line in source_list.read_text(encoding="utf-8").splitlines()
                     if line.strip()}
        img_stems = [stem for stem in img_stems if stem in requested]
        missing_requested = requested - set(img_stems)
        if missing_requested:
            raise ValueError(f"Source list contains {len(missing_requested)} missing images")
    ...
    labelled_count = sum(bool(labels.get(stem)) for stem in img_stems)
    print(f"Found {len(img_stems)} source images: {labelled_count} labelled, "
          f"{len(img_stems) - labelled_count} unlabelled negatives")
    ...
    # A frozen source manifest is mandatory for final experiments.  It keeps
    # calibration/test sources isolated from training and hard-negative mining.
    if split_manifest:
        split_data = json.loads(split_manifest.read_text(encoding="utf-8"))
        raw_splits = split_data.get("splits", split_data)
        split_sets = {
            name: set(stems) & set(img_stems)
            for name, stems in raw_splits.items()
            if name in {"train", "val", "calibration", "test"}
        }
        if "calibration" in split_sets:
            split_sets.pop("val", None)
        if only_split:
            if only_split not in split_sets:
                raise ValueError(f"Requested split {only_split!r} is absent from manifest")
            split_sets = {only_split: split_sets[only_split]}
        required = {only_split} if only_split else {"train", "calibration", "test"}
        if not required.issubset(split_sets):
            raise ValueError(f"Split manifest must contain {sorted(required)}")
        assigned = set().union(*split_sets.values())
        if assigned != set(img_stems):
            missing = sorted(set(img_stems) - assigned)
            raise ValueError(f"Split manifest leaves {len(missing)} images unassigned: {missing[:5]}")
        if sum(len(s) for s in split_sets.values()) != len(assigned):
            raise ValueError("Source leakage detected across frozen splits")
    else:
        shuffled = sorted(img_stems)
        random.shuffle(shuffled)
        split_idx = max(1, int(len(shuffled) * (1 - val_ratio)))
        split_sets = {"train": set(shuffled[:split_idx]), "val": set(shuffled[split_idx:])}
    print("Split: " + " / ".join(f"{len(v)} {k}" for k, v in split_sets.items()) +
          " source images")
    ...
    if dry_run:
        ...
        for stem in img_stems:
            ...
            split_tag = next(k for k, v in split_sets.items() if stem in v)
            stats["positive_crops"][split_tag] += len(entries)
            if split_tag == "train":
                expected_negatives = (negatives_per_image if entries
                                      else unlabelled_negatives_per_image)
            else:
                expected_negatives = (eval_negatives_per_image if entries
                                      else min(eval_negatives_per_image,
                                               unlabelled_negatives_per_image))
            stats["negative_crops"][split_tag] += expected_negatives
    ...
    for split_tag, stems in split_sets.items():
        ...
        for idx, stem in enumerate(sorted(stems)):
            ...
            neg_count = 0
            if split_tag == "train":
                target_negatives = (negatives_per_image if entries
                                    else unlabelled_negatives_per_image)
            else:
                target_negatives = (eval_negatives_per_image if entries
                                    else min(eval_negatives_per_image,
                                             unlabelled_negatives_per_image))
        ...
        print(f"  [{split_tag}] DONE: {stats['positive_crops'][split_tag]} positive + "
              f"{stats['negative_crops'][split_tag]} negative crops, {box_count} boxes")
    ...
    if not dry_run:
        config = {
            ...
            "train": "train/images",
            "val": ("calibration/images" if "calibration" in split_sets else "val/images"),
            "nc": NC,
            "names": CLASS_NAMES,
            "# crop_size": crop_size,
        ...
        for split_name in ("calibration", "test"):
            ...
            split_cfg["train"] = f"{split_name}/images"
            split_cfg["val"] = f"{split_name}/images"
            split_yaml = output_dir / f"{split_name}.yaml"
            if yaml is not None:
                split_yaml.write_text(
                    yaml.dump(split_cfg, default_flow_style=False, allow_unicode=True),
                    encoding="utf-8",
                )
            else:
                split_yaml.write_text(
...
""",
    ...
    parser.add_argument(
        ...
        help="Frozen JSON source split with train/calibration/test lists.",
    )
    parser.add_argument(
        "--eval-negatives-per-image", type=int, default=10,
        help="Negative crops per calibration/test source (default: 10).",
    )
    parser.add_argument(
        "--unlabelled-negatives-per-image", type=int, default=3,
        help="Crops per wholly unlabelled source (default: 3; prevents negative flooding).",
    )
    parser.add_argument("--source-list", type=Path, default=None,
                        help="Optional source-stem list for parallel sharding.")
    parser.add_argument("--only-split", choices=["train", "calibration", "test"], default=None,
                        help="Generate only one frozen split (for parallel sharding).")
    parser.add_argument(
    ...
    stats = generate_crops(
        ...
        eval_negatives_per_image=args.eval_negatives_per_image,
    ...
    print(f"{'=' * 60}")
    total_pos = sum(stats["positive_crops"].values())
    total_neg = sum(stats["negative_crops"].values())
    total_boxes = sum(stats["defect_boxes"].values())
    for split_name in stats["positive_crops"]:
        pos = stats["positive_crops"][split_name]
        neg = stats["negative_crops"][split_name]
        print(f"  {split_name:11s}: {pos} pos + {neg} neg = {pos + neg}")
