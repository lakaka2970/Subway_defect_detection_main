...
cp "$CFG" "$BACKUP.current_resume" 2>/dev/null || true
trap 'cp "$BACKUP.current_resume" "$CFG" 2>/dev/null || true' EXIT
...
raise SystemExit(0 if json.loads(Path(sys.argv[1]).read_text()).get("status") == "passed" else 1)
PY
}
...
preflight_gate phase5_resume_preflight

for variant in s3_a s3_b s3_d s3_h s3_f; do
  out="$RUN/phase5/screen/$variant"
  mkdir -p "$out"
  if json_passed "$RUN/audits/phase5_screen_${variant}/eval.json"; then
    echo "SKIP $variant: train/eval gates already passed"
    continue
  fi
  preflight_gate "phase5_screen_${variant}_preflight"
  write_config "$variant" 30 0
  cp "$CFG" "$out/config.yaml"
  if [ ! -s "$out/train/stage_3/weights/best.pt" ] || [ ! -s "$out/train/stage_3/results.csv" ]; then
  ...
  if [ ! -s "$out/calibration_eval/evaluation.json" ]; then
...
if not passing:
 raise SystemExit('No Stage3 screen variant passed selection constraints')
...
for seed in 0 1 2; do
  ...
  mkdir -p "$out"
  if json_passed "$RUN/audits/phase5_full_${selected}_seed${seed}/eval.json"; then
    echo "SKIP full $selected seed $seed: train/eval gates already passed"
    continue
  fi
...
if [ ! -s "$RUN/phase5/test/evaluation.json" ]; then
